...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
		...
		const buttonsDefault = [
			...
			{ quickReplyButton: { displayText: `💰Donasi`, id: `${prefix}sewabot` } },
		...
		switch(command) {
		    ...
		    { urlButton: { displayText: `Source Code`, url : `${setting.youtubeOwner}` } },
			{ callButton: { displayText: `Contact Me`, phoneNumber: `${setting.ownerNumber}` } },
			{ quickReplyButton: { displayText: `💳 Payment`, id: `${prefix}payment` } },
			{ quickReplyButton: { displayText: `🤖 Owner`, id: `${prefix}sewabot` } },
			{ quickReplyButton: { displayText: `💰 Donasi`, id: `${prefix}donate` } }
		]
			conn.sendMessage(from, { caption: `*Get Bot*\n\n\n Rp. 5.000 For One Group`, image: pp_bot, templateButtons: buttons, footer: `${setting.ownerName}`, mentions: [sender] })
			break
			case prefix+'payment':
			let txr =`Payment Method`
			let buttos = [
			{ quickReplyButton: { displayText: `Qris AllPay`, id: `${prefix}qris` } },
			{ quickReplyButton: { displayText: `Gopay`, id: `${prefix}gopay` } },
			{ quickReplyButton: { displayText: `Dana`, id: `${prefix}dana` } }
		]
			conn.sendMessage(from, { caption: txr, image: pp_bot, templateButtons: buttos, footer: `${setting.ownerName}`, mentions: [sender] })
            break
            case prefix+'qris':
            button = [
            { quickReplyButton: { displayText: `Back`, id: `${prefix}payment` } }
		]
            conn.sendMessage(from, { caption: `QRIS`, image: { url : 'https://telegra.ph/file/01d8580d17f5deb63afb6.jpg'}, templateButtons: button, footer: `${setting.ownerName}`, mentions: [sender] })
            break
            case prefix+'gopay':
            let button = [
            { quickReplyButton: { displayText: `Back`, id: `${prefix}payment` } }
		]
            conn.sendMessage(from, { caption: `GOPAY`, image: { url : 'https://telegra.ph/file/0648f288805d3773a552f.jpg'}, templateButtons: button, footer: `${setting.ownerName}`, mentions: [sender] })
            break
            case prefix+'dana':
            reply(`Dana : 081266891985`)
            break
			case prefix+'menu':
			case prefix+'help':
			...
			case prefix+'owner':
			    for (let x of ownerNumber) {
			      sendContact(from, x.split('@s.whatsapp.net')[0], 'Satganz Devs 😷', msg)
			    }
			    break
			case prefix+'cekprem':
