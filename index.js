          ...
          if (isWelcome) {
            try {
              for (let i of data.participants) {
                ...
                if (data.action == "add") {
                	let welton = [
			{ quickReplyButton: { displayText: `Irashaimase!!`, id: `` } },
			{ quickReplyButton: { displayText: `                                           @6281316701742 Ganteng`, id: `${prefix}jela` } }
		]
                  conn.sendMessage(data.id, { image: { url: pp_user }, caption: `Welcome @${i.split("@")[0]}`, welton, mentions: [i] })
                } else if (data.action == "remove") {
                	let byeton = [
			{ quickReplyButton: { displayText: `Sayonara!!`, id: `` } },
			{ quickReplyButton: { displayText: `                                           @6281316701742 Ganteng`, id: `${prefix}jela` } }
		]
                  conn.sendMessage(data.id, { image: { url: pp_user }, caption: `Sayonara @${i.split("@")[0]}`, byeton, mentions: [i] })
                }
              }
            } catch (e) {
              console.log(e)
